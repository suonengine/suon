...
fn benchmark_path_push_pop(c: &mut Criterion) {
    ...
    for queued_steps in [2usize, 16usize, 128usize] {
        group.bench_with_input(BenchmarkId::new("push_pop", queued_steps), &queued_steps, |b, &queued_steps| {
            b.iter(|| {
        ...
        group.bench_with_input(BenchmarkId::new("clear", queued_steps), &queued_steps, |b, &queued_steps| {
            b.iter(|| {
                let mut path = StepPath::default();
                for index in 0..queued_steps {
                    let direction = if index % 2 == 0 {
