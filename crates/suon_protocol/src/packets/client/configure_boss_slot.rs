...
/// # Examples
/// ```
/// use suon_protocol::packets::client::{Decodable, prelude::ConfigureBossSlotPacket};
///
/// let mut payload: &[u8] = &[2, 0x78, 0x56, 0x34, 0x12];
/// let packet = ConfigureBossSlotPacket::decode(&mut payload).unwrap();
///
/// assert_eq!(packet.slot, 2);
/// assert_eq!(packet.boss_race_id, 0x12345678);
/// ```
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct ConfigureBossSlotPacket {
...
impl Decodable for ConfigureBossSlotPacket {
    const KIND: PacketKind = PacketKind::ConfigureBossSlot;
...
mod tests {
    ...
    fn should_decode_configure_boss_slot() {
        ...
        let packet = ConfigureBossSlotPacket::decode(&mut payload)
