...
/// # Examples
/// ```
/// use suon_protocol::packets::client::{Decodable, PacketKind, prelude::PassPartyLeadership};
///
/// let mut payload: &[u8] = &[0x78, 0x56, 0x34, 0x12];
/// let packet = PassPartyLeadership::decode(PacketKind::PassPartyLeadership, &mut payload).unwrap();
///
/// assert_eq!(packet.target_id, 0x12345678);
/// ```
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct PassPartyLeadership {
    /// Creature id of the player who should become the new party leader.
    pub target_id: u32,
}

impl Decodable for PassPartyLeadership {
...
mod tests {
    ...
    fn should_decode_pass_party_leadership() {
        ...
        let packet = PassPartyLeadership::decode(PacketKind::PassPartyLeadership, &mut payload)
